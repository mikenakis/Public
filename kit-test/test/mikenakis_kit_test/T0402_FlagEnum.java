...
public class T0402_FlagEnum
{
	...
	@Test public void FlagEnum_Checks_Out()
	{
		...
		FlagEnumSet<E> empty = flagEnum.of();
		assert empty.isEmpty();
		assert empty.getBits() == 0;
		FlagEnumSet<E> ab = flagEnum.of( E.A, E.B );
		assert !ab.isEmpty();
		assert ab.getBits() == 0x0003;
		FlagEnumSet<E> abc = ab.tryWith( E.C );
		assert !abc.isEmpty();
		assert abc.getBits() == 0x0007;
		assert abc.equals( flagEnum.of( E.A, E.B, E.C ) );
		FlagEnumSet<E> b = ab.tryWithout( E.A );
		assert !b.isEmpty();
		...
		Map<FlagEnumSet<E>,E> map = new HashMap<>();
