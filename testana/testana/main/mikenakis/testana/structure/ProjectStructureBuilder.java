...
public final class ProjectStructureBuilder
{
		...
		StructureSettings settings, Optional<Path> cachePathName, Iterable<TestEngine> testEngines )
	{
		...
		Map<String,TestEngine> testEngineMap = Kit.collection.stream.streamFromIterable( testEngines ).collect( Collectors.toMap( testEngine -> testEngine.name(), testEngine -> testEngine ) );
